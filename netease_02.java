...
public class Main{
    public static void main(String[] args){
        ...
        int[] tx = new int[taxiNum];
        int[] ty = new int[taxiNum];
        ...
        for(int i=0;i<taxiNum;i++){
            taxi = Math.min(taxi,(Math.abs(tx[i])+Math.abs(ty[i]))*walkTime + (Math.abs(gx-tx[i])+Math.abs(gy-ty[i]))*taxiTime);
        }
        System.out.println(Math.min(taxi,walk));
    }
}